...
public class DelayedOperation {
    ...
    public DelayedOperation(Context context, String tag, long delayInMilliseconds) {
        ...
        Logger.d(LOG_TAG, "created delayed operation with tag: " + mTag);
    }
    ...
    public void start(final Operation operation) {
        ...
        Logger.d(LOG_TAG, "starting delayed operation with tag: " + mTag);
    ...
    public void resetTimer() {
        ...
        Logger.d(LOG_TAG, "resetting delayed operation with tag: " + mTag);
        mTimer = new Timer();
        mTimer.schedule(new TimerTask() {
            @Override
            public void run() {
                if (mOperation.shouldExecuteDelayedOperation()) {
                    Logger.d(LOG_TAG, "executing delayed operation with tag: " + mTag);
    ...
    public void cancel() {
        if (mTimer != null) {
            Logger.d(LOG_TAG, "cancelled delayed operation with tag: " + mTag);
            mTimer.cancel();
            mTimer = null;
        }
