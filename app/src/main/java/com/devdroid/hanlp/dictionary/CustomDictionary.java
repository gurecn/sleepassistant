 ...
 * </copyright>
 */
package com.devdroid.hanlp.dictionary;
...
import com.devdroid.hanlp.collection.trie.bintrie.BinTrie;
...
import com.devdroid.hanlp.utility.LexiconUtility;
...
public class CustomDictionary
{
    ...
    static
    {
        String path[] = HanLP.CustomDictionaryPath;
    ...
    public static boolean load(String path, Nature defaultNature, TreeMap<String, CoreDictionary.Attribute> map, LinkedHashSet<Nature> customNatureCollector)
    {
        try
        {
            ...
            while ((line = br.readLine()) != null)
            {
                ...
                if (HanLP.Normalization) param[0] = CharTable.convert(param[0]); // 正规化
...
//                if (updateAttributeIfExist(param[0], attribute, map, rewriteTable)) continue;
    ...
    {
        if (HanLP.Normalization) word = CharTable.convert(word);
        if (contains(word)) return false;
    ...
    {
        if (HanLP.Normalization) key = CharTable.convert(key);
    ...
    /**
     * 热更新（重新加载）<br>
     * 集群环境（或其他IOAdapter）需要自行删除缓存文件（路径 = HanLP.CustomDictionaryPath[0] + Predefine.BIN_EXT）
     * @return 是否加载成功
     */
