...
public class SimplifiedChineseDictionary extends BaseChineseDictionary
{
    ...
    static
    {
        ...
        if (!load(HanLP.tcDictionaryRoot + "s2t.txt", trie, false))
        {
            throw new IllegalArgumentException("简繁词典" + HanLP.tcDictionaryRoot + "s2t.txt" + Predefine.BIN_EXT + "加载失败");
        }

        logger.info("简繁词典" + HanLP.tcDictionaryRoot + "s2t.txt" + Predefine.BIN_EXT + "加载成功，耗时" + (System.currentTimeMillis() - start) + "ms");
    }
